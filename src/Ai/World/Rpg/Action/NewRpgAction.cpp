...
bool StartRpgDoQuestAction::Execute(Event event)
{
    ...
    if (quest)
    {
        ...
        bot->Whisper(botAI->GetLocalizedBotTextOrDefault("msg_start_do_quest", "Start to do quest %id", {{"%id", std::to_string(questId)}}), LANG_UNIVERSAL, owner);
        return true;
    }
    bot->Whisper(botAI->GetLocalizedBotTextOrDefault("error_invalid_quest_whisper", "Invalid quest %text", {{"%text", text}}), LANG_UNIVERSAL, owner);
    return false;
}
