...
bool EquipGlyphsAction::Execute(Event event)
{
    ...
    {
        botAI->TellMaster(botAI->GetLocalizedBotTextOrDefault("msg_glyph_usage", "Usage: glyph equip <6 glyph item IDs> (3 major, 3 minor)."));
        return false;
    }
    ...
    for (auto const* g : glyphs)
    {
        ...
        if (!placed)
        {
            botAI->TellMaster(botAI->GetLocalizedBotTextOrDefault("error_glyph_no_sockets", "Not enought empty sockets for all glyphs."));
            return false;
        }
    }

    botAI->TellMaster(botAI->GetLocalizedBotTextOrDefault("msg_glyphs_updated", "Glyphs updated."));
