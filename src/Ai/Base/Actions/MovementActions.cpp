...
bool MovementAction::Follow(Unit* target, float distance, float angle)
{
    ...
    if (bot->GetTransport() && target->GetTransport() == bot->GetTransport())
    {
        TransportFollowHelper::AdjustFollowOnSharedTransport(bot, target, distance);
        return false;
    }
...
bool MovementAction::MoveToForTransportFollow(uint32 mapId, float x, float y, float z, MovementPriority priority)
{
    return MoveTo(mapId, x, y, z, false, false, false, true, priority, true);
}

void MovementAction::SetTransportFollowDelay(float delayMillis)
{
    SetNextMovementDelay(delayMillis);
}
