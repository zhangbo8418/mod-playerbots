...
bool ListSpellsAction::Execute(Event event)
{
    ...
    if (spells.empty())
    {
        ...
        botAI->TellMaster(botAI->GetLocalizedBotTextOrDefault("msg_no_spells_found", "No spells found."));
        return true;
    }

    botAI->TellMaster(botAI->GetLocalizedBotTextOrDefault("msg_spells_title", "=== Spells ==="));
