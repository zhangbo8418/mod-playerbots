...
bool FollowAction::Execute(Event /*event*/)
{
    ...
    if (!leader)
    {
        if (Unit* groupLeader = AI_VALUE(Unit*, "group leader"))
            leader = groupLeader->ToPlayer();
    }

    if (leader && TransportFollowHelper::HandleFollowTransport(bot, leader, botAI, this) ==
        TransportFollowHelper::FollowResult::Handled)
        return true;
