        ...
        if (newMaster)
        {
            ...
            if (!bot->InBattleground())
            {
                ...
                if (botAI->GetMaster() == botAI->GetGroupLeader())
                    ...
                    botAI->TellMaster(!urand(0, 2)
                        ? botAI->GetLocalizedBotTextOrDefault("hello", "Hello!")
                        : botAI->GetLocalizedBotTextOrDefault("hi", "Hi!"));
            }
            else
            {
...
void PlayerbotAI::Reset(bool full)
{
    ...
    if (!logout && bot->GetSession()->isLogingOut())
    {
        WorldPackets::Character::LogoutCancel data = WorldPacket(CMSG_LOGOUT_CANCEL);
        bot->GetSession()->HandleLogoutCancelOpcode(data);
        TellMaster(GetLocalizedBotTextOrDefault("logout_cancel", "Logout cancelled!"));
    }
...
void PlayerbotAI::HandleCommand(uint32 type, std::string const text, Player* fromPlayer)
{
    ...
    else if (filtered == "logout")
    {
        ...
        if (fromPlayer != master)
        {
            if (type == CHAT_MSG_WHISPER)
            {
                bot->Whisper(GetLocalizedBotTextOrDefault(
                    "bot_not_your_master", "You are not my master!", {}, fromPlayer),
                    LANG_UNIVERSAL, fromPlayer);
            }
            return;
        }
        ...
        if (masterBotMgr->GetPlayerBot(bot->GetGUID()))
        {
            if (type == CHAT_MSG_WHISPER)
            {
                TellMaster(GetLocalizedBotTextOrDefault("logout_start", "I'm logging out!"));
            }
        ...
        {
            TellMaster(GetLocalizedBotTextOrDefault(
